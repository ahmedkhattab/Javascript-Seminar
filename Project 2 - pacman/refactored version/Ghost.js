...
Ghost.prototype.eyeWhite = function (size){
	ctx.fillStyle = "white"; 
	ctx.beginPath();//left eye
	ctx.arc(this.x-this.radius/2.5, this.y-this.radius/5, this.radius/size, 0, Math.PI*2, true); // white
	ctx.fill();
...
Ghost.prototype.eyeBlack = function (offsetX, offsetY){
	...
	ctx.beginPath();
	ctx.arc(this.x+offsetX, this.y+offsetY, this.radius/6, 0, Math.PI*2, true); //black
	ctx.fill();
};
...
Ghost.prototype.legsHelper = function (offset1, offset2){
		ctx.lineTo(this.x-this.radius, this.y+this.radius+offset1);
		ctx.lineTo(this.x-this.radius+this.radius/3, this.y+this.radius+offset2);
		ctx.lineTo(this.x-this.radius+this.radius/3*2, this.y+this.radius+offset1);
		ctx.lineTo(this.x, this.y+this.radius+offset2);
		ctx.lineTo(this.x+this.radius/3, this.y+this.radius+offset1);
		ctx.lineTo(this.x+this.radius/3*2, this.y+this.radius+offset2);
		ctx.lineTo(this.x+this.radius, this.y+this.radius+offset1);
...
Ghost.prototype.legs = function (){
		// LEGS
	if (!this.isMoving){
		this.legsHelper(0,-this.radius/4);
	}
	else {
		this.legsHelper(-this.radius/4,0);
	}
}
Ghost.prototype.draw = function() {
	...
	if(this.isWeak){
		...
		switch(this.dir){
			...
			case LEFT:
				this.eyeBlack(-this.radius/3-this.radius/5, -this.radius/5);
				this.eyeBlack(this.radius/3-this.radius/15, -this.radius/5);
			break;
